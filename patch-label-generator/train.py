...
path_patches = glob.glob(c.dataroot+"train/patches/*_random_*.gz")  # should be c.dataroot+"train/patches/*.gz"
path_labels = glob.glob(c.dataroot+"train/seg_labels/*_random_*.gz")  # should be c.dataroot+"train/seg_labels/*.gz"
...
for epoch in range(start_epoch, c.num_epochs+1):
    ...
    for i, data in enumerate(dataloader, 1):
        ...
        for k in range(c.n_disc):
            ...
            noise = torch.randn(b_size, c.nz, 1, 1, 1, dtype=torch.float32, device=device)

            # Training within the mixed-precision autocast - enabled/disabled
            with torch.cuda.amp.autocast(enabled=False):  # was enabled=c.use_mixed_precision
            ...
            # Clip the gradients
            # torch.nn.utils.clip_grad_norm_(netD.parameters(), 100)
        ...
        # Clip the gradients
        # torch.nn.utils.clip_grad_norm_(netG.parameters(), 100)
    ...
    for idx in sample_idx:
        ...
        if c.save_nifti:
            ut.convert_and_save_to_nifti(sample[:4].to(
