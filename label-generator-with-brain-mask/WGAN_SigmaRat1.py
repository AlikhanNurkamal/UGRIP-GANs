...
class Discriminator(nn.Module):
    def __init__(self, channel=512,out_class=1,is_dis =True):
        ...
        self.conv1 = SN(nn.Conv3d(1, channel//8, kernel_size=4, stride=2, padding=1))
        ...
        self.conv5 = SN(nn.Conv3d(channel, n_class, kernel_size=4, stride=1, padding=0))
...
class Generator(nn.Module):
    def __init__(self, noise:int=100, channel:int=64):
        ...
        self.noise = noise
        self.tp_conv1 = SN(nn.ConvTranspose3d(noise, _c*8, kernel_size=4, stride=1, padding=0, bias=False))
        self.bn1 = nn.InstanceNorm3d(_c*8)
        
        self.tp_conv2 = SN(nn.Conv3d(_c*8, _c*4, kernel_size=3, stride=1, padding=1, bias=False))
        self.bn2 = nn.InstanceNorm3d(_c*4)
        
        self.tp_conv3 = SN(nn.Conv3d(_c*4, _c*2, kernel_size=3, stride=1, padding=1, bias=False))
        self.bn3 = nn.InstanceNorm3d(_c*2)
        
        self.tp_conv4 = SN(nn.Conv3d(_c*2, _c, kernel_size=3, stride=1, padding=1, bias=False))
        self.bn4 = nn.InstanceNorm3d(_c)
        
        self.tp_conv5 = SN(nn.Conv3d(_c, 1, kernel_size=3, stride=1, padding=1, bias=False))
